...
int main(int argc, char **argv) {
  ...
  printf("Size target : %ld\n", sizeof(target));
  cl_mem target_mem_obj =
      clCreateBuffer(context, CL_MEM_READ_ONLY, 16, NULL, &ret);
  if (ret) goto free_queue;
  cl_mem solution_mem_obj =
      clCreateBuffer(context, CL_MEM_WRITE_ONLY, PWD_LEN + 1, NULL, &ret); // For the password I added CL_MEM_WRITE_ONLY
  if (ret) goto free_target_buf;

  // Copy the lists A and B to their respective memory buffers
  ret = clEnqueueWriteBuffer(command_queue, target_mem_obj, CL_TRUE, 0, // I want all the hash
